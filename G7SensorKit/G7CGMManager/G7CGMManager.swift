...
public class G7CGMManager: CGMManager {
    ...
    public var lifecycleState: G7SensorLifecycleState {
        if state.sensorID == nil {
            ...
            return state.sessionMode == .direct && canAuthenticate ? .connecting : .searching
        }
    ...
    public convenience init() {
        ...
        self.init(state: state, sensor: G7Sensor(mode: .eavesdropping, credentials: state.sensorCredentials))
    }
