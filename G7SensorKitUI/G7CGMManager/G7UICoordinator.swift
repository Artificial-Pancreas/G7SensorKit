...
class G7UICoordinator: UINavigationController, CGMManagerOnboarding, CompletionNotifying, UINavigationControllerDelegate {
    ...
    /// Handshake narration from a first-time pairing run, held until a manager
    /// exists to carry it into the device log. It is the single most useful
    /// trace when a pairing goes wrong in the field, and during initial
    /// setup there is nowhere else for it to go. Kept across retries; flushed
    /// once the manager is created. Capped so a long-running failure cannot
    /// grow without bound.
    private var pendingPairingLog: [String] = []
    private static let pendingPairingLogLimit = 400
    ...
    /// Re-pairing has a manager whose device log carries the narration
    /// directly; first-time setup holds it until one exists.
    private func recordPairingLog(_ message: String) {
        if let cgmManager = cgmManager {
            cgmManager.logDeviceCommunication("[pairing] " + message, type: .connection)
        } else {
            pendingPairingLog.append(message)
            if pendingPairingLog.count > G7UICoordinator.pendingPairingLogLimit {
                pendingPairingLog.removeFirst(pendingPairingLog.count - G7UICoordinator.pendingPairingLogLimit)
            }
        }
    }
    ...
    private func pairingSucceeded(code: String, peripheralIdentifier: UUID, sharedKey: Data, deviceName: String?, handoff: G7PairingHandoff?) {
        if let cgmManager = cgmManager {
            ...
            let manager = G7CGMManager(pairingCode: code, peripheralIdentifier: peripheralIdentifier, sharedKey: sharedKey, handoff: handoff)
            cgmManager = manager
            // Flush the held narration first, so the log reads in order:
            // handshake, then the session that follows from it.
            for message in pendingPairingLog {
                manager.logDeviceCommunication("[pairing] " + message, type: .connection)
            }
            pendingPairingLog.removeAll()
            cgmManagerOnboardingDelegate?.cgmManagerOnboarding(didCreateCGMManager: manager)
            cgmManagerOnboardingDelegate?.cgmManagerOnboarding(didOnboardCGMManager: manager)
        }
